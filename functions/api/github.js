...
// Proxies GitHub's REST API using a server-side token (env.GITHUB_TOKEN), so
// no GitHub credential ever needs to reach a browser. Requires our own
// dashboard auth (Authorization: Bearer <token> from /api/login).
...
export async function onRequestGet({ request, env }) {
  ...
  if (!env.GITHUB_TOKEN) return json({ error: 'GITHUB_TOKEN is not configured on the server yet.' }, { status: 500 });
  ...
  if (type === 'repos') {
    ...
    let ghRes = await fetch(`https://api.github.com/orgs/${owner}/repos?per_page=100&sort=updated`, { headers: ghHeaders(env) });
    if (ghRes.status === 404) {
      ghRes = await fetch(`https://api.github.com/users/${owner}/repos?per_page=100&sort=updated`, { headers: ghHeaders(env) });
    }
  ...
  const ghRes = await fetch(`https://api.github.com/repos/${repo}/${path}`, { headers: ghHeaders(env) });
  const data = await ghRes.json().catch(() => null);
  if (!ghRes.ok) {
